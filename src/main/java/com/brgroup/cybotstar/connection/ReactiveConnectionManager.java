...
public class ReactiveConnectionManager {
    ...
    private final ConcurrentHashMap<String, Mono<ReactiveWebSocketConnection>> connectionCache =
            new ConcurrentHashMap<>();

    public ReactiveConnectionManager(@NonNull AgentConfig config) {
        this.config = config;
    }
     ...
     */
    @NonNull
    public Mono<ReactiveWebSocketConnection> getConnection(@NonNull String sessionId) {
     ...
     */
    @NonNull
    private Mono<ReactiveWebSocketConnection> createConnection(@NonNull String sessionId) {
        return Mono.fromCallable(() -> {
            ReactiveWebSocketConnection connection = new ReactiveWebSocketConnection(config);
            return connection;
        })
        .flatMap(connection ->
            connection.connect()
    ...
    public Mono<Void> disconnect(@NonNull String sessionId) {
        return Mono.defer(() -> {
            Mono<ReactiveWebSocketConnection> cachedConnection = connectionCache.remove(sessionId);
            if (cachedConnection == null) {
                return Mono.empty();
            }

            return cachedConnection
                    .doOnNext(ReactiveWebSocketConnection::close)
                    .then()
