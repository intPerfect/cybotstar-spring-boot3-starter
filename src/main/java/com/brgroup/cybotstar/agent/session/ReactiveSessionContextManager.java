...
import com.brgroup.cybotstar.connection.ReactiveWebSocketConnection;
 ...
 */
@Slf4j
public class ReactiveSessionContextManager {

    @NonNull
    private final ReactiveConnectionManager connectionManager;

    // 会话上下文缓存
    private final ConcurrentHashMap<String, Mono<ReactiveSessionContext>> contextCache =
            new ConcurrentHashMap<>();

    public ReactiveSessionContextManager(@NonNull ReactiveConnectionManager connectionManager) {
     ...
     */
    @NonNull
    public Mono<ReactiveSessionContext> getContext(@NonNull String sessionId) {
     ...
     */
    @NonNull
    private Mono<ReactiveSessionContext> createContext(@NonNull String sessionId) {
        return connectionManager.getConnection(sessionId)
                .map(connection -> new ReactiveSessionContext(sessionId, connection))
    ...
    public Mono<Void> removeContext(@NonNull String sessionId) {
        return Mono.defer(() -> {
            Mono<ReactiveSessionContext> cachedContext = contextCache.remove(sessionId);
            if (cachedContext == null) {
                return Mono.empty();
            }

            return cachedContext
                    .doOnNext(ReactiveSessionContext::close)
                    .then()
